...
import { mapOfferWithDetails } from "../mappers.js";
 ...
 */
export async function getOffersByEsimId(esimId: string): Promise<OfferWithDetails[]> {
  ...
  return rows.map(mapOfferWithDetails);
}
 ...
 */
export async function getOfferById(id: string): Promise<OfferWithDetails | null> {
  const rows = await sql`
    SELECT
      o.id,
      o.esim_id,
  ...
  return rows[0] ? mapOfferWithDetails(rows[0]) : null;
}